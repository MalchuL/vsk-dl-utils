...
def PerceptualLossSimple(model_name='vgg19_bn', loss_type='charbonnier', use_last_layers=None, z_clip=3, fix_pad=True,
                         apply_norm=True):
    name2layers = {'vgg16': VGG16_LAYERS,
                   'vgg19': VGG19_LAYERS,
                   'vgg19_bn': VGG19_BN_LAYERS}
    ...
    return PerceptualLoss(model_name=model_name, layers=layers, apply_norm=apply_norm, fix_pad=fix_pad, mean=mean,
                          std=std,
                          loss_type=loss_type, z_clip=z_clip)


class PerceptualLoss(nn.Module):
    # Layers from https://towardsdatascience.com/implementing-neural-style-transfer-using-pytorch-fd8d43fb7bfa
    def __init__(self, model_name='vgg19', layers=(), weight_scaler=2, apply_norm=False, fix_pad=False, mean=None,
                 std=None, reverse_weights=False, loss_type='smooth_l1', z_clip=None):
        ...
        self.z_clip = z_clip
        self.perception = self.get_model(model_name=model_name,
                                         layers=layers,
                                         fix_pad=fix_pad,
                                         mean=self.mean,
                                         std=self.std)
    ...
    def get_model(self, model_name, layers=(), fix_pad=False, mean=None, std=None):
        return VGGFeatures(network=model_name, layers=layers, fix_pad=fix_pad, mean=mean,
                           std=std, z_clipping=self.z_clip)
    ...
    def get_norm(self, num_channels):
        return nn.InstanceNorm2d(num_channels, affine=False,
                                 track_running_stats=False)  # Please don't touch eps in norm, it affects image gray content
