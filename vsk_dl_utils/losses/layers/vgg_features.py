...
class VGGFeatures(nn.Module):
    ...
    def __init__(self, network='vgg19_bn', layers=None, mean=None, std=None, fix_pad=False,
                 pooling='max', z_clipping=None):
        """
        VGG features extractor
        :param network: VGG network name
        :param layers: Layers numbers to extract.
        :param mean: Mean list to normalize image. By default as in ImageNet
        :param std: Std list to normalize image. By default as in ImageNet
        :param fix_pad: Makes confolutions with Reflect padding
    ...
    def forward(self, x):
        """
        Calculates VGG features
        :param x: 4D images tensor. Inputs must be normalized in [0..1] range
    ...
    def extra_repr(self) -> str:
        return f'Norm: [mean: {self.mean.view(3)}, std: {self.std.view(3)}]\n' +\
