...
class ChannelsPool(nn.Module):
    def __init__(
        ...
        self.stats = nn.ModuleList([layers[pool_layer] for pool_layer in pool_layers])
        ...
        def get_act():
            return nn.LeakyReLU(
                negative_slope=NEG_SLOPE, inplace=True
            )  # alpha because at -1 gradient lokks like leaky relu

        self.mlp = nn.Sequential(
            conv1x1(in_channels, hidden_layers, groups=groups, bias=False),
...
class CartoonGANDiscriminator(nn.Module):
    def __init__(
        ...
        def get_act(inplace=True):
            return nn.LeakyReLU(
                negative_slope=NEG_SLOPE, inplace=inplace
            )  # alpha because at -1 gradient lokks like leaky relu
        ...
        def get_norm(channels, affine=False):
            return (
                ...
                if apply_instance_norm
                    ...
                    channels, affine=affine, eps=self.eps, track_running_stats=False
                )
            )
        ...
        for i in range(len(strides)):
            if i in mlp:
                self.mlp[str(len(modules))] = ChannelsPool(
                    out_channels[i],
                    out_channels[i],
                    1,
                    eps=self.eps,
                    channels_as_groups=channels_as_groups,
                    groups=groups,
                )
        ...
        )  # You must have bias as true to good converge
...
class StyleCartoonGANDiscriminator(nn.Module):
    def __init__(
        self,
        style_layers=("color_shift_with_sobel", "surface", "identity"),
        weight_mode="uniform",
        r=5,
        eps=2e-1,
        layers=(32, 64, 128, 128, 256, 256),
        ...
        super().__init__()
        self.style_representation = StyleRepresentation(
            layers=style_layers, weight_mode=weight_mode, r=r, eps=eps
        )
        self.discriminator = CartoonGANDiscriminator(
            input_channels=self.style_representation.get_num_channels(),
            layers=layers,
            mlp=mlp,
            strides=strides,
